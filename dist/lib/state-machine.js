function setState(state, nextState, now) {
...
export function applyHookEvent(previous, input, now) {
    let state = {
        ...previous,
        lastUpdatedAt: now.toISOString(),
    ...
    switch (input.hook_event_name) {
        ...
        case "SubagentStop": {
            ...
            return setState({
                ...state,
                activeSubagentCount
            }, activeSubagentCount > 0 ? "subagent_running" : "thinking", now);
        }
        case "Stop":
...
function applyNotification(state, notificationType, now) {
    const nextState = (() => {
