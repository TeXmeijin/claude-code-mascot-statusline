...
const HEAT_MAX = 85;
const HEAT_TARGET = [0xff, 0x44, 0x44];
const HEAT_PALETTE_INDEX = 2;
export function applyContextHeatPalette(palette, usedPercentage) {
...
export async function renderStatusLine(input, options = {}) {
    ...
    const heatPalette = applyContextHeatPalette(pack.manifest.sprite.palette, heatSource);
...
export function renderSprite(pack, sprite, options) {
    ...
    if (renderMode === "half-block" && renderProfile === "claude-code-safe") {
        return applyHorizontalOffset(renderHalfBlockSpriteSafe(centerHalfBlockSprite(sprite), palette, options.colorEnabled, options.safeBackground ?? "#000000"), offsetX);
    }
    if (renderMode === "half-block") {
        return applyHorizontalOffset(renderHalfBlockSprite(centerHalfBlockSprite(sprite), palette, options.colorEnabled), offsetX);
    }
    const pixelWidth = options.narrow ? 1 : pack.manifest.sprite.pixelWidth ?? 2;
    return applyHorizontalOffset(renderBgSpaceSprite(sprite, palette, {
...
function resolveEffectiveState(sessionState, pack, now) {
    if (!sessionState) {
        return "idle";
    }
    ...
    if (sessionState.currentState === "done" ||
...
function selectMostRecentState(persistedState, transcriptState) {
    if (!persistedState) {
...
function renderBgSpaceSprite(sprite, palette, options) {
...
function renderHalfBlockSprite(sprite, palette, colorEnabled) {
    const rows = [];
    for (let rowIndex = 0; rowIndex < sprite.length; rowIndex += 2) {
        const top = sprite[rowIndex] ?? [];
        const bottom = sprite[rowIndex + 1] ?? new Array(top.length).fill(0);
        let line = "";
...
function renderCoalescedBgSprite(sprite, palette, options) {
    const merged = [];
    for (let rowIndex = 0; rowIndex < sprite.length; rowIndex += 2) {
        const top = sprite[rowIndex] ?? [];
...
function renderHalfBlockSpriteSafe(sprite, palette, colorEnabled, safeBackground) {
    const rows = [];
    for (let rowIndex = 0; rowIndex < sprite.length; rowIndex += 2) {
        const top = sprite[rowIndex] ?? [];
        const bottom = sprite[rowIndex + 1] ?? new Array(top.length).fill(0);
...
function chooseMergedPixel(topIndex, bottomIndex, palette) {
...
function applyHorizontalOffset(rendered, width) {
    if (width <= 0) {
        return rendered;
    }
...
function centerHalfBlockSprite(sprite) {
...
function shiftSpriteRow(row, shift, width) {
...
function renderHalfBlockCell(top, bottom, colorEnabled) {
    if (!colorEnabled) {
        if (top === null && bottom === null) {
            return " ";
        }
...
function renderHalfBlockSafeCell(top, bottom, colorEnabled, safeBackground) {
    if (top === null && bottom === null) {
        if (!colorEnabled) {
            return " ";
        }
...
function colorizeByHeat(text, percentage, colorEnabled) {
...
function truncate(value, widthHint) {
    if (widthHint === null) {
        return value;
    }
