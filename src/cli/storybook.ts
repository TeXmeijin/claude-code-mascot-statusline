...
import { MASCOT_STATES, type LoadedPack, type MascotState } from "../lib/types.js";
...
async function resolveTargetPacks(args: ReturnType<typeof parseArgs>): Promise<LoadedPack[]> {
  if (args.dir) {
...
function renderPackHeader(pack: LoadedPack): string {
  const lines = [
...
function renderPackIndex(packs: LoadedPack[]): string {
